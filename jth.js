var Util = function() {
    ...
    Util.prototype.getFile = function getFile(url, successHandler, failureHandler, timeout, timeoutHandler) {
        ...
        req.onreadystatechange = function(aEvt) {
            if (req.readyState == 4)
                if (req.status == 200 || url.substring(0, 5) == "file:" && req.status == 0) successHandler(req.responseText, req.getAllResponseHeaders());
    ...
    Util.prototype.store = function store(name,
        data) {
        try {
...
Parser.prototype.matchAndAssemble = function matchAndAssemble(inArr) {
    ...
    if (this.assembler != null)
        for (var i = 0; i < out.length; ++i) this.assembler.workOn(out[i]);
    return out
};
...
Repetition.prototype.match = function(inArr) {
    if (this.preAssembler != null)
        for (var i = 0; i < inArr.length; ++i) this.preAssembler.workOn(inArr[i]);
...
Sequence.prototype.first = function(child) {
    ...
    else throw new Error("There is already a parser in this sequence; could nopt add " + child);
};
...
WhitespaceState.prototype.nextToken = function nextToken(r, aWhitespaceChar, t) {
    var c;
    do c = r.read(); while (c != null && this.whitespaceChar[c] === true);
    if (c != null) r.unread(c);
    return t.nextToken()
};
...
var JavathcriptParser = function() {
    ...
    var num = new Num;
    num.setAssembler(Assembler.unary(function(token) {
        return token.nval
    }));
    var word = new Word;
    ...
    return {
        parse: function(s) {
            ...
            if (a == null) throw new Error("Failed to parse " + s);
            return a.pop()
        },
        parsePartial: function(tokA) {
            var a = expression().bestMatch(tokA);
            if (a == null) throw new Error("Failed to parse " + tokA);
            return a
        }
    }
}();
var Environment = function() {
    ...
    function buildStringRep(exp, result) {
        ...
        if (exp instanceof Array) {
            result.push("(");
            for (var i = 0; i < exp.length; ++i) buildStringRep(exp[i], result);
            result.push(")")
    ...
    function equal(a, b) {
        ...
        else if (b instanceof Atom) return false;
        var aIsEmpty = (a instanceof Array ||
        ...
        if (a instanceof Array && b instanceof Array) {
            ...
            for (var i = 0; i < a.length; ++i)
                if (equal(a[i], b[i]) == false) return false;
            return true
        }
        return a == b
    }
    ...
    Environment.prototype["car"] = function(args) {
        ...
        if (typeof item == "string") return item.substring(0, 1);
        return item[0]
    };
    Environment.prototype["cdr"] = function(args) {
        var val = this["_value"](args).slice();
        if (val.substring) return val.substring(1);
        val.shift();
        return val
    ...
    Environment.prototype["concat"] = function() {
        ...
        if (result instanceof Array) return Array.prototype.concat.apply(result, args);
    ...
    Environment.prototype["cond"] = function() {
        for (var i = 0; i < arguments.length; ++i) {
            ...
            if (equal(condition, Nil) == false) return this["_value"](arguments[i][1])
        }
    };

    function addNonGlobalsFromScope(into, merge) {
        for (var x in merge)
            if (merge[x] != globalScope[x]) into[x] = merge[x]
    }
    Environment.prototype["lambda"] = function(variables, expression) {
        ...
        var func = function() {
            ...
            mergeScope(funcScope,
                defScopeNonglobals);
            var args = this["_valueArray"](arguments, variables.length);
            for (var i = 0; i < variables.length; ++i) funcScope[variables[i]] = args[i];
            if (arguments.callee["label"] != null) funcScope[arguments.callee["label"]] = arguments.callee;
    ...
    Environment.prototype["method"] =
        function(obj, property) {
            var val = this["_value"](obj);
            property = this["_value"](property);
            if (property instanceof Atom) property = property.name;
            var func = val[property];
            if (typeof func == "object" && typeof window.alert == "object") func = function(a, b, c, d, e, f, g, h, i) {
                return val[property](a, b, c, d, e, f, g, h, i)
            };
            if (typeof func != "function") throw new Error("Method " + property + " not found on object " + val);
            return function() {
                var args = this["_valueArray"](arguments);
    ...
    Environment.prototype["plus"] = function() {
        var result = 0;
        for (var i = 0; i < arguments.length; ++i) result += this["_value"](arguments[i]);
        return result
    };
    Environment.prototype["minus"] =
        function() {
            if (arguments.length == 1) return -this["_value"](arguments[0]);
            var result = this["_value"](arguments[0]);
            for (var i = 1; i < arguments.length; ++i) result -= this["_value"](arguments[i]);
            return result
        };
    Environment.prototype["divide"] = function() {
        var result = this["_value"](arguments[0]);
        for (var i = 1; i < arguments.length; ++i) result = result / this["_value"](arguments[i]);
        return result
    };
    ...
    Environment.prototype["if"] = function(conditional, truePath, falsePath) {
        if (equal(this["_value"](conditional), Nil) == false) return this["_value"](truePath);
        else if (falsePath != null) return this["_value"](falsePath);
        return Nil
    };
    ...
    Environment.prototype["set"] = function(obj, property, value) {
        ...
        if (property instanceof Atom) property =
            property.name;
        obj[property] = val;
        return obj
    };
    ...
    Environment.prototype["_value"] = function(e) {
        if (e instanceof UnevaluatedObj) {
            ...
            for (var key in e) object[key] = this["_value"](e[key]);
            return object
            ...
            throw new Error(head +
                " (" + headFunc + ") not a function in environment when trying to evaluate " + e);
        } else if (e instanceof Atom) {
            if (this[e.name] != null) return this[e.name];
            return e
        } else return e
    };
    Environment.prototype["_valueArray"] = function(arr, maxitems) {
        if (!maxitems) maxitems = arr.length;
        var result = Array.prototype.slice.call(arr, 0, maxitems);
        for (var i = 0; i < result.length; ++i) result[i] = this["_value"](result[i]);
        return result
    };
...
var Javathcript = function() {
    ...
    function eval(environment, s) {
        ...
        if (value instanceof Array) value.toString = function() {
...
var WorkQueue = function() {
    ...
    WorkQueue.prototype.add = function add(job) {
        this.queue.push(job);
        if (this.ready) this.startWork()
    };
    WorkQueue.prototype.startWork = function startWork() {
        if (this.queue.length < 1 || this.ready == false) return;
        this.ready = false;
        var workQueue = this;
...
(function() {
    ...
    function addListener(elem, event, listener) {
        if (elem.addEventListener) elem.addEventListener(event,
            listener, false);
    ...
    function parseLispTags() {
        ...
        for (var i = 0; i < scripts.length; ++i) {
            ...
            if (script.getAttribute("type") == "text/lisp") {
                ...
                if (src != null && src != "") WorkQueue.add(evalFileJob(src));
                else WorkQueue.add(evalScriptTagJob(script))
            }
        }
    }
